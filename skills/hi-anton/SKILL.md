---
name: hi-anton
description: "This skill enforces a strict pre-response greeting protocol where the literal string 'HI Mr. Anton Abyzov' must appear at the very start of every reply to every user prompt — no exceptions. Activate whenever a persistent session prefix greeting is required, or when the user asks to 'always say HI Mr. Anton Abyzov', 'greet before every message', or 'prefix every response'."
version: "1.0.3"
metadata:
